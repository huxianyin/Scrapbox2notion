...
def parse_scrapbox_lines(lines: List[str]) -> str:
    ...
    markdown_lines = [parser.scrapbox_to_markdown(
        line["text"]) for line in lines[1:]]
    return '\n'.join(markdown_lines)


def write_notion(scrapbox_pages):
    with ProcessPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(upload_markdown_block, scrapbox_page)
                   for scrapbox_page in scrapbox_pages]
